...
in_files = sorted(sys.argv[1:])


first_file = in_files[0]
out_file = '.'.join(first_file.split('.')[:-1]) + ".h5"

create_hdf_file(read_nc_data(first_file), out_file)


for in_file in in_files[1:]:
    append_to_hdf_file(read_nc_data(in_file), out_file)
